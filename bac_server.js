...
app.get("/configured/:config/trakt-action", async (req, res) => {
  ...
  console.log(`[TRAKT-ACTION] Click detected!`);
  ...
  const requestKey = `${config}-${action}-${type}-${imdbId}-${season || ''}-${episode || ''}-${rating || ''}`;
  const now = Date.now();
  ...
  if (!lastRequest || (now - lastRequest) > FIVE_MINUTES) {
    ...
    setTimeout(async () => {
      try {
        const userConfig = decodeConfig(config);
        if (userConfig && userConfig.access_token) {
          console.log(`[TRAKT-BACKGROUND] Starting ${action} request...`);
          ...
          if (result.success) {
            console.log(`[TRAKT-BACKGROUND] ✅ ${result.message}`);
          } else {
            console.error(`[TRAKT-BACKGROUND] ❌ Failed: ${result.error}`);
          }
        } else {
          console.error('[TRAKT-BACKGROUND] Invalid or missing config/access token');
        }
      } catch (error) {
        console.error(`[TRAKT-BACKGROUND] Unexpected error: ${error.message}`);
      }
    }, 100);
  } else {
    const timeSince = (now - lastRequest) / 1000;
    console.log(`[TRAKT-ACTION] Skipping duplicate request (made ${Math.floor(timeSince)}s ago)`);
  }
